...
import {async_all, ensure_timestamp, assert_logger} from 'mixpanel/lib/utils.js';
import {MixpanelGroups} from 'mixpanel/lib/groups.js';
import {MixpanelPeople} from 'mixpanel/lib/people.js';
...
export function init(token, config) {
    if (!token) {
     ...
     */
    metrics.send_request = async function(options, callback) {
        ...
        const content = Buffer.from(JSON.stringify(options.data)).toString('base64');
        ...
        const url = metrics.config.protocol + "://" + metrics.config.host + (metrics.config.port ? ":" + metrics.config.port : "") + metrics.config.path + endpoint + "?" + query_params.toString();

        try {
            const response = await fetch(url, {
                method: method,
                headers: headers,
                body: method === 'POST' ? content : null,
                keepalive: metrics.config.keepAlive
            });
            ...
            if (metrics.config.debug) {
                metrics.config.logger.error("Got Error: " + error.message);
            }
            callback(error);
        }
    };
