...
export function init(token, config) {
    ...
    metrics.send_request = async function(options, callback) {
        ...
        let content = new URLSearchParams(JSON.stringify(options.data));
        ...
        const url = metrics.config.protocol + "://" + metrics.config.host + (metrics.config.port ? ":" + metrics.config.port : "") + metrics.config.path + endpoint + "?" + (method === 'GET' ? content.toString() : "");
